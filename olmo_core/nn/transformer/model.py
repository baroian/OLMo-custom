...
class Transformer(nn.Module):
    ...
    @staticmethod
    def make_divisible(value, devisor):
        # This is how corenet does it
        if value < devisor:
            return devisor
        rounded = int(value + devisor/2) // devisor * devisor
        # round up if more then 10% lower
        if rounded <= 0.9 * value:
            return rounded + devisor
        else:
            return rounded
